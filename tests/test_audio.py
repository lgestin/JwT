...
from tts.data.audio import Audio, AudioInfo


def test_audio_from_file_loads_waveform(audio_from_file: Audio) -> None:
...
def test_random_audio_basic_properties(random_audio: Audio) -> None:
...
def test_audio_info_roundtrip(audio_from_file: Audio) -> None:
    info = audio_from_file.info
    assert isinstance(info, AudioInfo)
    rebuilt = Audio.from_audioinfo(info)
...
def test_loudness_is_finite(audio_from_file: Audio) -> None:
...
def test_mono_collapses_channels() -> None:
    ...
    audio = Audio(waveform=waveform, sample_rate=16000).mono()
...
def test_salient_excerpt_returns_expected_duration(audio_from_file: Audio) -> None:
...
def test_audio_from_waveform_requires_sample_rate() -> None:
    with pytest.raises(AssertionError):
        Audio(waveform=torch.zeros(1, 16))


def test_device_property(random_audio: Audio) -> None:
    assert random_audio.device == random_audio.waveform.device


def test_stft_property_shape(random_audio: Audio) -> None:
