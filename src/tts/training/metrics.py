"""Diagnostic metric helpers for the trainer.

These run in the training hot loop, so they are deliberately sync-free: every
function returns GPU tensors and never calls `.item()`/`.cpu()`. The trainer
accumulates the results on-device and performs a single batched host transfer
per logging window.
"""
